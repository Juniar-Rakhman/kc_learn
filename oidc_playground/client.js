...
function generateAuthenticationRequest() {
  ...
  setOutput(
    ...
    req.replace("?", "<br/><br/>").replaceAll("&", "<br/>"),
  );
    ...
    if (req.readyState === 4) {
      ...
      if (response["id_token"]) {
        var idToken = JSON.parse(
          base64UrlDecode(response["id_token"].split(".")[1]),
        );
...
function userInfo() {
  ...
  setOutput(
    "output-userInfoRequest",
    state.discovery["userinfo_endpoint"] +
    "<br/><br/>" +
...
function step(step) {
  ...
  for (i = 0; i < steps.length; i++) {
    document.getElementById("step-" + steps[i]).style.display =
      steps[i] === step ? "block" : "none";
  }
  setState("step", step);
